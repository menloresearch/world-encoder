...
class LeJEPAVisionHead(nn.Module):
    """Vision-only LeJEPA with a FROZEN ViT backbone + a tiny trainable head — the
    properly-matched baseline to the multimodal Perceiver (which also freezes the
    backbone and trains a small head). The head is exactly the Perceiver's vision
    projection ``proj_v`` (768 -> d), and the per-view embedding is
    ``proj_v(patch_tokens).mean(1)`` — identical to the multimodal online ``ev``.

    Only ``proj_v`` (~200k params at d=256) trains; the backbone runs in ``no_grad``
    eval mode each step. Loss is the SAME library path as LeJEPAVideo:
    ``LeJEPA._compute_loss`` over the multi-crop view embeddings + ``SlicedEppsPulley``.
    """

    def __init__(self, d=256, pretrained=True, n_slices=1024, lamb=0.02):
        super().__init__()
        self.backbone = ViTv2Backbone(pretrained=pretrained)
        self.backbone.requires_grad_(False)
        self.backbone.eval()
        self.proj_v = nn.Linear(self.backbone.embed_dim, d)  # == Perceiver proj_v (768->d)
        self.sigreg = SlicedEppsPulley(num_slices=n_slices)
        self.lamb, self.d = lamb, d

    def train(self, mode=True):
        super().train(mode)
        self.backbone.eval()                                 # keep backbone frozen/eval always
        return self

    def _embed_group(self, views):
        """Same-size views [B,3,H,W]*n -> [n*B, d] = proj_v(patch).mean(patches).
        Backbone is frozen (no_grad); grad flows only through proj_v."""
        with torch.no_grad():
            patch = self.backbone.model(torch.cat(views))["patch_latent"]  # [n*B, P, 768]
        return self.proj_v(patch).mean(1)                    # [n*B, d]

    def forward(self, global_views, local_views=None):
        local_views = local_views or []
        # globals (224 -> 196 patches) and locals (96 -> fewer patches) have different
        # patch counts, so each size group runs through the backbone separately; the
        # mean-pooled [., d] embeddings are then concatenated view-major.
        g = self._embed_group(global_views)                  # [n_g*B, d]
        feats = g if not local_views else torch.cat([g, self._embed_group(local_views)])
        B = global_views[0].shape[0]
        n_views = len(global_views) + len(local_views)
        proj = feats.view(n_views, B, -1)                    # [V, B, d]
        loss, inv, sig = LeJEPA._compute_loss(proj, len(global_views), self.sigreg, self.lamb)
        return {"loss": loss, "inv_loss": inv.detach(), "sigreg_loss": sig.detach(),
                "embedding": g.detach()}
    ...
    @torch.no_grad()
    def embed(self, images):
        """z_v-style vision embedding [N, d] for probing: proj_v(patch).mean(1)."""
        patch = self.backbone.model(images)["patch_latent"]  # [N, P, 768]
        return self.proj_v(patch).mean(1)
