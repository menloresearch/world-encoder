...
# 1. extract frames — all robot scenes (--scene <name> for one; --include-human for demos)
...
python -m world_tokenizer.make_shards --num-workers 48
...
CUDA_VISIBLE_DEVICES=1,2,3,4,5,6,7 python -m torch.distributed.run --nproc_per_node=7 \
    -m world_tokenizer.train --shards /mnt/nas/data/RH20T/cfg3_shards --epochs 10 --lr 2e-5
#   debug on 1 scene/1 GPU:  --frames-root <scene_dir> --max-steps 30

# 5. eval — scene-held-out, health + robot-relevant (all take --ckpt-template for custom paths)
